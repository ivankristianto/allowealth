/**
 * Account Priority Calculation Tests
 * ================================
 * Unit tests for account priority utilities
 */
...
import {
  ...
  filterAccountsNeedingUpdate,
  getAccountsByPriority,
  countAccountsByPriority,
  getNextUpdateDate,
  isUpdateOverdue,
  PRIORITY_THRESHOLDS,
...
describe('calculateAccountPriority', () => {
  it('should return high priority for >30 days', () => {
    ...
    const result = calculateAccountPriority(thirtyOneDaysAgo);
  ...
  it('should return medium priority for >14 days', () => {
    ...
    const result = calculateAccountPriority(fifteenDaysAgo);
  ...
  it('should return low priority for >7 days', () => {
    ...
    const result = calculateAccountPriority(tenDaysAgo);
  ...
  it('should return none priority for <=7 days', () => {
    ...
    const result = calculateAccountPriority(fiveDaysAgo);
  ...
  it('should return none priority for today', () => {
    ...
    const result = calculateAccountPriority(today);
    expect(result.priority).toBe('none');
  ...
  it('should return high priority for null lastUpdated', () => {
    const result = calculateAccountPriority(null);
    expect(result.priority).toBe('high');
  ...
  it('should return high priority for undefined lastUpdated', () => {
    const result = calculateAccountPriority(undefined);
    expect(result.priority).toBe('high');
    expect(result.daysSinceUpdate).toBe(Number.MAX_SAFE_INTEGER);
    expect(result.needsUpdate).toBe(true);
    expect(result.lastUpdated).toBe(null);
  ...
  it('should handle exact threshold boundaries', () => {
    ...
    const result30 = calculateAccountPriority(thirtyDaysAgo);
    expect(result30.priority).toBe('medium');

    const fourteenDaysAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000);
    const result14 = calculateAccountPriority(fourteenDaysAgo);
    expect(result14.priority).toBe('low');

    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const result7 = calculateAccountPriority(sevenDaysAgo);
    expect(result7.priority).toBe('none');
  });
});

describe('needsAccountUpdate', () => {
  it('should return true for accounts needing update', () => {
    const tenDaysAgo = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000);
    expect(needsAccountUpdate(tenDaysAgo)).toBe(true);
  });

  it('should return false for recently updated accounts', () => {
    const fiveDaysAgo = new Date(Date.now() - 5 * 24 * 60 * 60 * 1000);
    expect(needsAccountUpdate(fiveDaysAgo)).toBe(false);
  });

  it('should return true for null lastUpdated', () => {
    expect(needsAccountUpdate(null)).toBe(true);
  });

  it('should return true for undefined lastUpdated', () => {
    expect(needsAccountUpdate(undefined)).toBe(true);
  });
});
...
describe('getPriorityValue', () => {
  ...
  it('should handle all priority types', () => {
    const priorities: AccountPriority[] = ['high', 'medium', 'low', 'none'];
...
describe('sortAccountsByPriority', () => {
  ...
  it('should sort accounts by priority descending', () => {
...
describe('filterAccountsNeedingUpdate', () => {
  interface TestAccount {
    id: string;
    lastUpdated: Date | null;
  }

  it('should filter accounts that need update', () => {
    const accounts: TestAccount[] = [
      { id: '1', lastUpdated: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000) }, // none
    ...
    const filtered = filterAccountsNeedingUpdate(accounts);
  ...
  it('should handle empty array', () => {
    const filtered = filterAccountsNeedingUpdate([]);
    expect(filtered).toHaveLength(0);
  });
});

describe('getAccountsByPriority', () => {
  interface TestAccount {
    id: string;
    lastUpdated: Date | null;
  }

  it('should filter accounts by priority level', () => {
    const accounts: TestAccount[] = [
      { id: '1', lastUpdated: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000) }, // none
      { id: '2', lastUpdated: new Date(Date.now() - 35 * 24 * 60 * 60 * 1000) }, // high
      { id: '3', lastUpdated: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000) }, // medium
    ...
    const highPriority = getAccountsByPriority(accounts, 'high');
    expect(highPriority).toHaveLength(2);
    expect(highPriority.every((a) => a.id === '2' || a.id === '4')).toBe(true);

    const mediumPriority = getAccountsByPriority(accounts, 'medium');
...
describe('countAccountsByPriority', () => {
  interface TestAccount {
    id: string;
    lastUpdated: Date | null;
  }

  it('should count accounts by priority level', () => {
    const accounts: TestAccount[] = [
      { id: '1', lastUpdated: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000) }, // none
      { id: '2', lastUpdated: new Date(Date.now() - 35 * 24 * 60 * 60 * 1000) }, // high
      { id: '3', lastUpdated: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000) }, // medium
    ...
    const counts = countAccountsByPriority(accounts);
  ...
  it('should handle empty array', () => {
    const counts = countAccountsByPriority([]);
