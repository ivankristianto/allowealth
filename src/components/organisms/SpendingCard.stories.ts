...
export const Default: Story = {
  ...
  render: (args) => html`
    <spending-card
      ...
      remaining-label="${args.remainingLabel}"
      alert-message="${args.alertMessage}"
...
customElements.define(
  ...
  class extends HTMLElement {
    connectedCallback() {
      ...
      const remainingLabel = this.getAttribute('remaining-label') ?? 'Remaining';
      ...
      const formatCurrency = (amount: string | number, curr: string, compact = false): string => {
        ...
        if (compact && numericAmount >= 1_000_000) {
          return `${config.symbol}${(numericAmount / 1_000_000).toFixed(1)}M`;
        }
