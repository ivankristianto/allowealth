/**
 * AssetFormModal Storybook Stories
 *
 * P1: NOTE - This file duplicates the AssetFormModal.astro HTML structure because
 * Storybook's HTML framework cannot directly render Astro components.
 * When updating AssetFormModal.astro, ensure this file is updated to match.
 *
 * @see src/components/organisms/AssetFormModal.astro
 */
import type { Meta, StoryObj } from '@storybook/html';

const meta: Meta = {
  title: 'Organisms/AssetFormModal',
  tags: ['autodocs'],
  argTypes: {
    mode: {
      control: 'select',
    ...
    assetName: {
      control: 'text',
      description: 'Pre-filled asset name (edit mode)',
    },
    assetType: {
      control: 'select',
      options: ['bank_account', 'mutual_fund', 'bond', 'crypto', 'stock', 'other'],
      description: 'Pre-selected asset type',
    },
    currency: {
      control: 'select',
      options: ['IDR', 'USD'],
...
const createAssetFormModal = (args: AssetFormModalArgs): HTMLElement => {
  const {
    mode = 'add',
    assetName = '',
    ...
    isLoading = false,
    hasError = false,
    errorMessage = 'An error occurred while saving the asset.',
  } = args;
          ...
          <div class="space-y-2">
            ...
            <select class="select select-lg w-full bg-base-200 border border-base-300 rounded-2xl font-bold text-base-content focus:ring-2 focus:ring-accent focus:outline-none focus:border-accent">
              ${Object.entries(assetTypeLabels)
                .map(
                  ([value, label]) =>
                    `<option value="${value}" ${value === assetType ? 'selected' : ''}>${label}</option>`
                )
                .join('')}
            </select>
          </div>
...
export const AddMode: StoryObj = {
  ...
  render: (args) => createAssetFormModal(args),
};
...
export const AllAssetTypes: StoryObj = {
  args: {
    mode: 'add',
    assetName: '',
    assetType: 'stock',
