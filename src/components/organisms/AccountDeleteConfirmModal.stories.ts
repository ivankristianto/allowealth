/**
 * AccountDeleteConfirmModal Storybook Stories (Deactivate Account Modal)
 *
 * P1: NOTE - This file duplicates the AccountDeleteConfirmModal.astro HTML structure because
 * Storybook's HTML framework cannot directly render Astro components.
 * When updating AccountDeleteConfirmModal.astro, ensure this file is updated to match.
 *
 * @see src/components/organisms/AccountDeleteConfirmModal.astro
 */
import type { Meta, StoryObj } from '@storybook/html';
import { formatCurrency } from '@/lib/formatting/currency-client';

const meta: Meta = {
  title: 'Organisms/AccountDeleteConfirmModal',
  tags: ['autodocs'],
  argTypes: {
    accountName: {
      control: 'text',
      description: 'Name of the account to deactivate',
    },
    accountType: {
      control: 'text',
...
interface AccountDeactivateModalArgs {
  accountName?: string;
  accountType?: string;
  balance?: number;
  currency?: Currency;
...
const createAccountDeactivateModal = (args: AccountDeactivateModalArgs): HTMLElement => {
  const {
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="bg-base-200 rounded-2xl p-4">
          <div class="flex items-center justify-between">
            <div>
              <p class="font-bold text-base-content">${accountName}</p>
              <p class="text-xs text-base-content/50 mt-1">${accountType}</p>
            </div>
...
export const Default: StoryObj = {
  ...
  render: (args) => createAccountDeactivateModal(args),
};
...
export const WithError: StoryObj = {
  args: {
    accountName: 'Investment Accounts',
