/**
 * Asset Categories Renderer
 *
 * DOM rendering functions for the asset categories page.
...
import type { AssetCategoryHtmlResponse } from '@/lib/api/assetCategoryApiClient';
...
export function showLoadingState(): void {
  const table = document.querySelector('[data-testid="asset-categories-table"]');
  if (table) {
...
export function renderCategoryTableHtml(html: string): void {
  ...
  if (!tableBody) {
    console.error('[AssetCategoriesRenderer] [data-category-table-body] not found!');
    return;
  }
  ...
  if (!table) {
    console.error('[AssetCategoriesRenderer] Parent table not found!');
    return;
  }
 ...
 */
export function renderFromHtmlResponse(response: AssetCategoryHtmlResponse): void {
