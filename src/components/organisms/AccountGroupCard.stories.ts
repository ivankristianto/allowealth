/**
 * AccountGroupCard Storybook Stories
 *
 * P1: NOTE - This file duplicates the AccountGroupCard.astro HTML structure because
 * Storybook's HTML framework cannot directly render Astro components.
 * When updating AccountGroupCard.astro, ensure this file is updated to match.
 *
 * @see src/components/organisms/AccountGroupCard.astro
 */
import type { Meta, StoryObj } from '@storybook/html';
import { formatCurrency } from '@/lib/formatting/currency-client';

const meta: Meta = {
  title: 'Organisms/AccountGroupCard',
  tags: ['autodocs'],
  argTypes: {
    type: {
      control: 'text',
    ...
    count: {
      control: 'number',
      description: 'Number of accounts in group',
    },
    totalIdr: {
      control: 'number',
...
const createAccountRow = (account: AccountItem): string => {
  const currencyBadgeClasses =
    account.currency === 'IDR' ? 'bg-success/10 text-success' : 'bg-info/10 text-info';
  const currencyValueClasses = account.currency === 'IDR' ? 'text-success' : 'text-info';

  return `
    <div class="p-4 lg:px-8 lg:py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-base-100/50 transition-all group" data-account-row="${account.id}">
      <div class="flex-1 min-w-0">
        <h4 class="text-base lg:text-lg font-bold text-base-content leading-none tracking-tight truncate">${account.name}</h4>
        <div class="flex items-center gap-3 mt-2 lg:mt-3">
          <span class="px-2 py-0.5 rounded-md text-xs font-bold tracking-wider uppercase ${currencyBadgeClasses}">${account.currency}</span>
          <span class="text-xs text-base-content/50 font-medium tracking-tight">Updated: ${account.lastUpdated}</span>
        </div>
      </div>
      <div class="flex items-center gap-6 lg:gap-10">
        ...
        <div class="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
          ...
          <button class="p-2 text-base-content/50 hover:text-success transition-all rounded-xl hover:bg-success/10" title="Edit account">
          ...
          <button class="p-2 text-base-content/50 hover:text-error transition-all rounded-xl hover:bg-error/10" title="Delete account">
...
const createGroupCard = (args: GroupCardArgs): HTMLElement => {
  ...
  container.innerHTML = `
    <div class="bg-base-100 rounded-3xl border border-base-300 shadow-sm overflow-hidden transition-all duration-300" data-account-group="${groupId}">
      <button
        type="button"
...
const bankAccountAccounts: AccountItem[] = [
...
const stockAccounts: AccountItem[] = [
