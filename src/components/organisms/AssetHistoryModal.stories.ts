/**
 * AssetHistoryModal Storybook Stories
 *
 * P1: NOTE - This file duplicates the AssetHistoryModal.astro HTML structure because
 * Storybook's HTML framework cannot directly render Astro components.
 * When updating AssetHistoryModal.astro, ensure this file is updated to match.
 *
...
interface AssetHistoryModalArgs {
  assetName?: string;
  assetType?: string;
  balance?: number;
  currency?: 'IDR' | 'USD';
...
const createAssetHistoryModal = (args: AssetHistoryModalArgs): HTMLElement => {
  const {
      ...
      <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h2 class="text-2xl font-bold tracking-tight text-base-content leading-none">${assetName}</h2>
              <p class="text-xs font-bold text-base-content/40 uppercase tracking-widest mt-2">${assetType} • ${currency} Portfolio</p>
            </div>
          </div>
...
export const Default: StoryObj = {
  ...
  render: (args) => createAssetHistoryModal(args),
};
