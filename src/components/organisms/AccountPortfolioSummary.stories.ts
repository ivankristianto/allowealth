/**
 * AccountAccountsSummary Storybook Stories
 *
 * NOTE: This file duplicates the AccountAccountsSummary.astro HTML structure because
 * Storybook's HTML framework cannot directly render Astro components.
 * When updating AccountAccountsSummary.astro, ensure this file is updated to match.
 *
 * @see src/components/organisms/AccountAccountsSummary.astro
 */
import type { Meta, StoryObj } from '@storybook/html';
import { formatCurrency } from '@/lib/formatting/currency-client';

const meta: Meta = {
  title: 'Organisms/AccountAccountsSummary',
  tags: ['autodocs'],
  argTypes: {
    totalIdr: { control: 'number', description: 'Total account value in IDR' },
    totalUsd: { control: 'number', description: 'Total account value in USD' },
...
const createAccountsSummary = (args: AccountsSummaryArgs): HTMLElement => {
  const {
    totalIdr = 300800000,
    totalUsd = 53000,
    debtIdr = 43200000,
  ...
  container.innerHTML = `
    <section class="bg-base-100 rounded-card border border-base-300 shadow-sm relative overflow-hidden" aria-label="Accounts overview summary">
      <!-- 3-Column Metrics: Accounts | Debt | Net Worth -->
        ...
        <div class="grid grid-cols-1 ${gridCols} gap-4">
          ...
          <div class="border-l-2 border-success rounded-xl bg-base-200/30 px-4 py-4">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-xs font-semibold uppercase tracking-widest text-base-content/50">Total Accounts</span>
            </div>
          ...
          <div class="border-l-2 border-accent rounded-xl bg-base-200/30 px-4 py-4">
            <div class="flex items-center gap-2 mb-3">
              ...
              <div class="tooltip tooltip-bottom" data-tip="Total accounts minus total debt. Positive means accounts exceed liabilities.">
                <button type="button" class="inline-flex items-center justify-center rounded-full text-base-content/30 hover:text-base-content/60 transition-colors" aria-label="What is net worth? Total accounts minus total debt.">
      ...
      <div class="px-6 lg:px-10 pb-6 lg:pb-8 space-y-8">
        ...
        ${
          ...
          <!-- Account Allocation -->
          <div class="space-y-4 pt-6 border-t border-base-200">
            <h3 class="font-bold text-sm uppercase tracking-wider text-base-content">Account Allocation</h3>

            <!-- Allocation Bar -->
            <div class="relative">
              <div class="h-5 w-full bg-base-300 rounded-full overflow-hidden flex shadow-inner" role="img" aria-label="Account allocation distribution">
                ${displayDistribution
                  .map(
                    (item) => `
...
export const Default: StoryObj = {
  ...
  render: (args) => createAccountsSummary(args),
};
...
export const ManyAccountTypes: StoryObj = {
  args: {
    totalIdr: 2000000000,
    totalUsd: 133333.33,
    debtIdr: 100000000,
