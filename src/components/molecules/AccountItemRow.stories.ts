/**
 * AccountItemRow Storybook Stories
 *
 * P1: NOTE - This file duplicates the AccountItemRow.astro HTML structure because
 * Storybook's HTML framework cannot directly render Astro components.
 * When updating AccountItemRow.astro, ensure this file is updated to match.
 *
 * @see src/components/molecules/AccountItemRow.astro
 */
import type { Meta, StoryObj } from '@storybook/html';
import { formatCurrency } from '@/lib/formatting/currency-client';

const meta: Meta = {
  title: 'Molecules/AccountItemRow',
  tags: ['autodocs'],
  argTypes: {
    name: {
      control: 'text',
...
const createAccountItemRow = (args: AccountItemRowArgs): HTMLElement => {
  const {
    id = 'test-id',
    name = 'BCA Checking',
    currency = 'IDR',
  ...
  container.innerHTML = `
    <div class="p-4 lg:px-8 lg:py-5 hover:bg-base-100/50 transition-all group cursor-pointer" data-account-row="${id}" role="button" aria-expanded="false">
      <!-- Desktop Layout -->
...
export const Default: StoryObj = {
  ...
  render: (args) => createAccountItemRow(args),
};

export const IDRAccount: StoryObj = {
  args: {
    id: '2',
    name: 'BCA Savings',
    currency: 'IDR',
