...
- [ ] **[VERY LOW]** Add `is:inline` directive to suppress Astro warnings in 6 files (or ignore)
- [ ] **[VERY LOW]** Fix Drizzle ORM deprecation warnings in schema files (upgrade when available)
- [ ] **[VERY LOW]** Remove unused `await` keywords in test files (getDb is synchronous)
